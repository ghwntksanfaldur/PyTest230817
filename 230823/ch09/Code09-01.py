...
# 가격이 제일 민감함,그래서,보통의 대기업에서는 따로 기술을 사용해서, 특정태그를 보호를 함
# 스크래핑을 못하게, 예)쿠팡에서 이런 기술을 사용

# 스크래핑을 할 사이트 정할 때, 영이 목적이 아니고, 누구나 자료를 봤을 때, 크게 상관없는 부분,
# 예)뉴스 라든지, 책 정보, 영화정보
# (동적 크롤링:셀레니움(많이 사용,크롬),외부API(네이버)를 이용해서 수집)

# 스마트 스토어, 키워드
# 기본적인 태그 요소를 이용해서,
# 자료수집
# 난이도 중,하 -> 가져올 요소의 파악하는데 시간이 걸림
# 기본적인 HTML 요소이해, DB(MySQL이해도),파이썬의 기본적 이해도
...
# 우리는 이렇게 사용하지 않음
# bs4 모듈을 이용해서, 우리 보기 좋게(가독성 좋게) 정리 해주는
# 모듈을 이용해서 작업을 합니다
